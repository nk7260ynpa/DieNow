...
The system SHALL 提供 `ProjectAgent` 類別，以 `anthropic` Python SDK（`messages.create`）呼叫 Claude 模型進行 pov_6 的每 tick 決策；模型預設為 `claude-sonnet-4-7`，可由環境變數 `PROJECT_AGENT_MODEL` 或 `ScenarioConfig.project_agent_model` 覆寫；ProjectAgent MUST 僅依賴 `LLMClient` 介面，測試期以 `FakeAnthropicClient` 替身執行。

#### Scenario: 正常呼叫 Anthropic API
- **GIVEN** `ANTHROPIC_API_KEY` 已設定、`PROJECT_AGENT_MODEL=claude-sonnet-4-7`
- **WHEN** pov_6 於 tick 5 呼叫 `project_agent.decide(observation)`
- **THEN** SDK 的 `messages.create` 被呼叫恰 1 次
- **AND** 請求參數包含 `model="claude-sonnet-4-7"`、system blocks、user messages、tool/structured output 設定
- **AND** 回傳值為解析後的 `Action` 物件

#### Scenario: 以 FakeAnthropicClient 可離線測試
- **GIVEN** 測試使用 `FakeAnthropicClient`，預先灌入 tick 5 → `PressAction(button_id=6)` 的回應
- **WHEN** `project_agent.decide(observation)` 被呼叫
- **THEN** 回傳 `PressAction(button_id=6)`
- **AND** 無任何外部網路呼叫發生

#### Scenario: API Key 未設定時啟動失敗
- **GIVEN** 未設定 `ANTHROPIC_API_KEY`、`LLMClient="anthropic"`
- **WHEN** scenario-runner 嘗試建立 `ProjectAgent`
- **THEN** 建構子 MUST raise `ConfigValidationError("ANTHROPIC_API_KEY is required")`
...
The system SHALL 為每次 LLM 呼叫設置預設 30 秒 timeout（可由 `ScenarioConfig.llm_timeout_seconds` 覆寫）；網路錯誤或逾時 MUST 被捕獲並轉為 `ActionParseError`（由 pov-manager 降級為 WaitAction）；連續 3 次 LLM 呼叫失敗 MUST raise `LLMUnavailableError` 並中止 scenario-runner（寫入 issues.md）。
...
The system SHALL 允許使用者在 `.env`、`ScenarioConfig` 或 CLI 參數中指定 LLM 模型；MUST 支援所有符合 `claude-sonnet-4-*`、`claude-opus-4-*`、`claude-haiku-4-*` 命名規則的模型；不合法的模型名 MUST 於啟動時 raise `ConfigValidationError`。
