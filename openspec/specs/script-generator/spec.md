...
The system SHALL 於關卡啟動前依序（pov_1 → pov_2 → pov_3 → pov_4 → pov_5）呼叫 LLM 產生 `script_1, script_2, script_3, script_4, script_5` 共 5 份劇本；每次呼叫輸入 MUST 包含：目標 pov 的 `persona`、世界環境描述（房間尺寸、6 body 起始位置、按鈕位置、戒指位置、面具／眼鏡設定、可用 action 原語清單）、當該 pov 有前世時的完整 `prior_life`（= 上一份劇本）；每份劇本輸出 MUST 為通過 Pydantic 驗證的 `Script` 物件。

#### Scenario: script_1 無前世、結構完整
- **WHEN** script-generator 以 `pov_1` 的 persona 呼叫 LLM 產生 `script_1`
- **THEN** 回傳 `Script(pov_id=1, prior_life=None, events=[...], persona=..., death_cause=...)`
- **AND** `events` 為時間排序（`t` 嚴格非遞減）、每筆 event 的 `actor` 必為合法 pov_id
- **AND** `events` 最後 MUST 為 `actor=pov_1, action_type="die"` 的事件（對應 pov_1 在關卡中最終死亡）
- **AND** `death_cause` 為 `press_wrong|ring_paradox|timeout` 其中之一
...
- **WHEN** LLM 首次回傳的文字無法被 Pydantic 解析為合法 `Script`
- **THEN** script-generator MUST 重試，且下一次 prompt 包含解析錯誤提示
- **AND** 若 3 次皆失敗 MUST raise `ScriptGenerationError` 並寫入 `issues.md`

### Requirement: 時間一致性驗證

The system SHALL 在每份 `script_n`（n ∈ {2,3,4,5}）生成後，立即對照 `script_{n-1}`（即 `prior_life`）執行時間一致性驗證；驗證通過的定義為：`script_n.events` 中所有 `actor == n-1` 的事件 OR `targets` 含 `n-1` 的事件，MUST 在 `script_{n-1}.events` 中以相同 `t`、相同 `action_type`、相同 `payload`、相同 `targets` 出現；任一不一致 → 驗證失敗；反之亦然（`script_{n-1}` 中 `actor/targets` 涉及 pov_n 的事件 MUST 在 `script_n` 中對應存在）。
...
The system SHALL 在任何 script 生成或驗證流程中達到 `max_retries`（預設 3，可由 `ScriptConfig.max_retries` 覆寫）仍失敗時，中止整個關卡啟動流程；MUST 將失敗摘要（失敗的 script_n、diff 內容、retry 次數、LLM 原始回應摘要）追加寫入 `openspec/changes/recreate-duannao-ring-of-hands/issues.md`（格式：`[Coordinator|Specialist] [<ISO-8601 時間戳>] [嚴重度: HIGH] <描述>`）；MUST raise 結構化例外供上層捕獲。
...
- **AND** `issues.md` 追加一筆 `HIGH` 嚴重度紀錄，含失敗 pov_id、diff、三次 LLM response id（若有）
...
The system SHALL 以介面 `LLMClient` 抽象所有 LLM 呼叫；生產環境使用 `AnthropicClient`（`anthropic` SDK）、測試環境使用 `FakeAnthropicClient`（回傳 stub）；script-generator MUST 僅依賴 `LLMClient` 介面，不得直接 import `anthropic` 以利離線測試。

#### Scenario: 以 FakeAnthropicClient 可完整產生 5 份劇本
- **GIVEN** `FakeAnthropicClient` 預先灌入 5 份合法 stub scripts
- **WHEN** script-generator 以 FakeAnthropicClient 執行
- **THEN** 5 份 script 皆能通過驗證
- **AND** 測試 MUST 可在無網路環境下通過

#### Scenario: 正式執行時讀 ANTHROPIC_API_KEY
- **WHEN** scenario-runner 以 `LLMClient="anthropic"` 啟動但 `ANTHROPIC_API_KEY` 未設定
